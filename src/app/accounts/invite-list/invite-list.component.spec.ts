...
import { Mock_GetAccountInvitesListResponse, Mock_InviteDTO } from '../../constants';
...
describe('InviteListComponent', () => {
  ...
  const dataSource: any = Mock_GetAccountInvitesListResponse.data.map((item) => {
    return {
      id: item.id,
      email: item.email,
  ...
  it('should call getList', () => {
    spyOn(registerService, 'getListInvite').and.returnValue(
    ...
    component['getList']();
    fixture.detectChanges();
    expect(registerService.getListInvite).toHaveBeenCalled();
  });
