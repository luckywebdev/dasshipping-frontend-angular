...
export class TabsComponent implements OnInit {
  ...
  private getState(name: string) {
    ...
    const states = STATES.filter(
      item =>
        item.name.toUpperCase() === nameState ||
        item.abbreviation.toUpperCase() === nameState,
    );
    const [state] = states;
    return state;
  }
  ...
  private getLocations(key: string) {
    ...
    if (this.validateLocation(location) && this.filterLocation[key]) {
      return JSON.stringify({
        ...
        radius: this.getValue(`${key}Radius`)
          ? this.getValue(`${key}Radius`)
          : 100,
      });
    }
  ...
  public search() {
    const filter = {
      ...
      vehicleType: this.getValue('carType'),
    };
  ...
  public importCsv(file: File) {
    ...
    this.leadService.import(file).subscribe(
      () => {
        setTimeout(
          () => {
