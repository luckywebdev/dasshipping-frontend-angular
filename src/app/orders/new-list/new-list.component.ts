...
import { IMAGES_MODEL } from '../../constants/imageCars';
...
export class NewListComponent implements OnInit, OnDestroy {
  ...
  private setData(order: OrderDTO) {
    ...
    const dispatcherFullName = this.getFullNameDispatcher(trip);
    const driverFullName = this.getFullNameDriver(trip);
    ...
    return {
      ...
      pick: `<div class="location_item"> ${sender.firstName || ''} ${sender.lastName || ''} <br> ${this.orderService.fullAddress(pickLocation)} <br>
      ${
        pickLocation && pickLocation.addressType ? `<span>${pickLocation.addressType}</span> <br>` : ''
        } ${sender.phoneNumber ? sender.phoneNumber + ' <br>' : ''} <span class="${this.lateDate(order, 'pickDate') ? 'late_date' : ''}">${this.orderService.toFormatDate(pickDate)}</span> </div>`,
      delivery: `<div class="location_item"> ${receiver.firstName || ''} ${receiver.lastName || ''} <br> ${this.orderService.fullAddress(deliveryLocation)} <br>
      ${
        deliveryLocation && deliveryLocation.addressType
          ? `<span>${deliveryLocation.addressType}</span> <br>`
          : ''
        }  ${receiver.phoneNumber ? receiver.phoneNumber + ' <br>' : ''} <span class="${this.lateDate(order, 'deliveryDate') ? 'late_date' : ''}">${this.orderService.toFormatDate(deliveryDate)}</span> </div>`,
      cargo: cars
        .map((car, index) => {
          const imageMake = IMAGES_MODEL.find((img) => car.make && img.name.toLowerCase() === car.make.toLowerCase());
          return `<div class="car_item">${index + 1}. ${imageMake && imageMake.url ? `<img class="car_logo" src="${imageMake.url}" />` : ''} ${car.year} ${car.make} ${car.model} ${car.type}</div>`;
        })
        .join('<br>'),
      shipper: `${shipper && shipper.companyName ? shipper.companyName : ''} <br> ${this.orderService.fullAddress(shipper)} <br> ${shipper.phone || ''}`,
      distance: distance ? this.orderService.formatDistance(distance) : '',
      price: salePrice ? this.orderService.formatPrice(salePrice) : '',
      pricePerMile: this.orderService.formatDistance(salePrice / distance),
      dispatcherId: order.dispatcherId || (trip && trip.dispatcherId),
      driverId: tripId ? trip.driverId : order.driverId,
      driver: tripId && trip.driverId && driverFullName ? `<div class="profile_item">${this.getAvatarDriver(trip)} ${driverFullName}</div>` : null,
      dispatcher: (order.dispatcherId || (trip && trip.dispatcherId)) && dispatcherFullName ? `<div class="profile_item">${this.getAvatarDispatcher(trip)} ${dispatcherFullName}</div>` : null,
      action: true,
  ...
  private getAvatarDriver(trip: TripDTO) {
    const avatar = trip && trip.driverId ? trip.driver.avatarUrl : null;
    const name = trip && trip.driverId ? trip.driver.firstName[0] : null;
    return avatar ? `<img class="avatar_item" src="${avatar}" />` : `<div class="no_avatar_item">${name}</div>`;
  }

  private getAvatarDispatcher(trip: TripDTO) {
    const avatar = trip && trip.dispatcherId ? trip.dispatcher.avatarUrl : null;
    const name = trip && trip.dispatcherId ? trip.dispatcher.firstName[0] : null;
    return avatar ? `<img class="avatar_item" src="${avatar}" />` : `<div class="no_avatar_item">${name}</div>`;
  }
      ...
      this.dataSource.data = this.dataSource.data.map((item) => {
        if (order.id === item.id) {
          ...
          return {
            ...item,
            tripId,
            driver: `<div class="profile_item">${this.getAvatarDriver(trip)} ${this.getFullNameDriver(trip)}</div>`,
            dispatcher: `<div class="profile_item">${this.getAvatarDispatcher(trip)} ${this.getFullNameDispatcher(trip)}</div>`,
          };
        }
        return item;
      });
        ...
        this.dataSource.data = this.dataSource.data.map((item) => {
          if (order.id === item.id) {
            return {
              ...item,
              tripId: result.id,
              driver: `<div class="profile_item">${this.getAvatarDriver(result)} ${this.getFullNameDriver(result)}</div>`,
              dispatcher: `<div class="profile_item">${this.getAvatarDispatcher(result)} ${this.getFullNameDispatcher(result)}</div>`,
              dispatcherId:
  ...
  public async goGeolocation(element: PeriodicElement) {
    const { user } = this.userService;
    this.loadingService.startLoading();
    if (
      ((user.id === element.dispatcherId && user.roleId === ROLES_STATUS.DISPATCHER) ||
      ...
      await this.router.navigate([APP_ROUTES.drivers_location, element.driverId]);
      this.loadingService.stopLoading();
    } else {
      this.notificationService.success(
        'This order is not still assigned to you',
